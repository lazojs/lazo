    ...
    return {
        ...
        _extensions: function () {
            ...
            var preResponseHandler = function (request, next) {
                ...
                if (response.isBoom) {
                    LAZO.logger.debug(['server._extensions'], 'Internal server error', request, response);
                    LAZO.logger.error(['server._extensions'], 'Internal server error', request.url.href, response.message, response.trace);
