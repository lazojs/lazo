define([
    ...
    'handlers/stream',
    'handlers/tunnel'
], function (appHandler, assetsHandler, cluster, ctlNavHandler, fs, hapi, path, resolver, safeHandler, streamHandler, tunnelHandler) {
    ...
    return {
            ...
            var preResponseHandler = function (request, next) {
                ...
                if (response.isBoom) {
                    ...
                    if (request.raw.req.headers['x-requested-with'] !== 'XMLHttpRequest') {
                        ...
                        if (errHandler) {
                            return next(request.generateView(errHandler.server, response.response.payload));
                        }
                    }
                }
        ...
        _stop: function () {
            ...
            try {
                ...
                LAZO.logger.error(['server.server._stop'], 'Error stopping server and disconnecting worker', err);
            }
        },
        ...
        _cluster: function () {
            ...
            if (!LAZO.CLUSTER) {
                this._worker();
                LAZO.logger.warn(['server.server._cluster'], 'Started connect server listening on port %d (SSL=%d)',
                    LAZO.PORT, LAZO.options.sslServer && LAZO.options.sslServer.port);
                return this;
            }
            ...
            if (cluster.isMaster) {
                ...
                cluster.on('disconnect', function (worker) {
                    LAZO.logger.warn(['server._cluster'], 'Worker %d:%d was disconnected.', worker.id, worker.process.pid);
                    cluster.fork();
                });
            } else {
                this._worker();
