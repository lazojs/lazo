define(['handlers/utils', 'handlers/app/processor'], function (utils, processor) {
    ...
    return function (request, route, svr) {
        ...
        LAZO.logger.debug(['server.handlers.app.main.processor.reply'], 'Processing route request...', route);

        processor.reply(route, params, {
            error: function (err) {
                err = err instanceof Error ? err : new Error(err);
                LAZO.logger.error(['server.handlers.app.main.processor.reply'], 'Error processing request...', route, err);
