    ...
    var ServiceProxy = Base.extend({
        ...
        get: function (svc, options) {
            ...
            options.error = function (err) {
                LAZO.logger.error(['server.serviceProxy.get'], 'Failed to get %s %j %j', svc, err.data, err);
                if (error) {
                    error(err);
                }
            };
        ...
        post: function (svc, attributes, options) {
            ...
            var error = options.error;
            options.error = function (err) {
                LAZO.logger.error(['server.serviceProxy.post'], 'Failed to post %s %j %j', svc, err.data, err);
                if (error) {
                    error(err);
                }
            };
        ...
        put: function (svc, attributes, options) {
            ...
            var error = options.error;
            options.error = function (err) {
                LAZO.logger.error(['server.serviceProxy.put'], 'Failed to put %s %j %j', svc, err.data, err);
                if (error) {
                    error(err);
                }
            };
        ...
        destroy: function (svc, attributes, options) {
            ...
            var error = options.error;
            options.error = function (err) {
                LAZO.logger.error(['server.serviceProxy.delete'], 'Failed to delete %s %j %j', svc, err.data, err);
                if (error) {
                    error(err);
                }
            };
