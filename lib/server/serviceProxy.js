define(['base', 'request', 'underscore', 'resolver/model'], function (Base, request, _, helpers) {
    ...
    var ServiceProxy = Base.extend({
        ...
        sync: function (method, model, options) {
            ...
            LAZO.logger.debug(['server.serviceProxy.sync'], 'Begin %s request', method, model, options);

            var errorWrapper = function (err, requestOptions, response, body) {
                LAZO.logger.debug(['server.serviceProxy.sync'], '%s request failed', method, model, options, err, response, body);
                LAZO.logger.error(['server.serviceProxy.sync'], '%s request failed', method, requestOptions.uri, err);
                ...
                request(requestOptions,
                    function (error, response, body) {
                        if (error) {
                            ...
                            LAZO.logger.debug(['server.serviceProxy.sync'], '%s request succeed', method, model, options, response, body);
