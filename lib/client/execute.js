define(
    [
        ...
        'renderer', 'context', 'utils/cmpProcessor', 'bundler', 'requestFilters', 'jquerycookie'
    ],
    function ($, _, rehydrate, prune, state, doc, prime, renderer, Context, cmpProcessor, Bundler, filter) {
        ...
        if ($.cookie('development') !== '1') {
            bundle.response(eventData.route, window.location, function (modules) {
                if (modules) {
                    ...
                    lazoRequire(rootCtx.dependencies.modules, function () {
                        process(eventData);
                    });
                } else {
                    process(eventData);
