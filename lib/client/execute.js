define(
    [
        ...
        'resolver/file', 'l!viewManager', 'jquerycookie'
    ],
    function ($, _, rehydrate, prune, state, doc, prime, renderer, Context, cmpProcessor, filter, file, viewManager) {

    'use strict';

    var bundle;
    var bundler = LAZO.initConf.bundler || 'lazoBundle';
                ...
                cmpProcessor.process({
                    ...
                    success: function (ctl, ctx, cmpDef) {
                        ...
                        if (modules) {
                            ...
                            cmpCss = bundle.createCSSLinks(cmpCss);
                            // remove old component css
                            ctx._rootCtx.dependencies.css = bundle.createCSSLinks(ctx._rootCtx.dependencies.css);
                            ctx._rootCtx.dependencies.css = state.cleanUpLinkDependencies(ctx._rootCtx.dependencies.css, cmpCss);
                            ...
                            cmpImports = bundle.createImportLinks(cmpImports);
                            ctx._rootCtx.dependencies.imports = bundle.createImportLinks(ctx._rootCtx.dependencies.imports);
                            ctx._rootCtx.dependencies.imports = state.cleanUpLinkDependencies(ctx._rootCtx.dependencies.imports, cmpImports);
                        }
                        ...
                        imports = state.getAddRemoveLinks('links');
                ...
                bundle.response(eventData.route, window.location.pathname, {
                    success: function (modules) {
                        if (modules) {
                            ...
                            rootCtx.dependencies.css = bundle.createCSSLinks(modules.css);
                            rootCtx.dependencies.imports = bundle.createImportLinks(modules.imports);
