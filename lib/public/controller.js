define([
    ...
    var Controller = Base.extend(_.extend({
        ...
        // container: the name of child component container
        // name: the name of the component to add
        // options:
        // - index: the location in the container array; default is a push
        // - success: callback after component has been added and rendered
        // - error: callback if adding or rendering fails
        // - params: context parameters
        // - render: false by default
        addChild: function (container, cmpName, options) {
            module.addChild(container, cmpName, this, {
                success: function (child) {
                    if (LAZO.app.isClient) {
                        // 1. find container; filter out child view results
                        // 2. get insert index and find location in the DOM
                        // 3. insert component markup

                        if (options.render) {

                        }

                        // viewManager.attachViews(child, function (err, resp) {
                        //     if (err) {
                        //         return options.error(err);
                        //     }
                        //     options.success(resp);
                        //     if (LAZO.app.isClient) {
                        //         child._getEl().removeClass('lazo-navigating').addClass('lazo-navigated');
                        //     }
                        // });
                    }
        ...
        remove: function (options) {
            this.parent.onChildRemove(this);
        },

        onChildRemove: function (child, options) {
            child.off('event:name');
        },
