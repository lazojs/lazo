define(['underscore', 'htmlparser', 'htmlparserToString', 'utils/module', 'jquery'],
            ...
            $widgets.each(function () {
                ...
                self._resolveWidget(name, {
                    success: function (Widget) {
                        ...
                        $(widget.el).removeClass('unbound').addClass('bound');
                        widget._uiStates = widget.getStates();
                        widget.afterRender({
                            success: function () {
                                $(widget.el).removeClass('rendering').addClass('rendered');
                            },
                            error: function (err) {
            ...
            for (var i = 0; i < list.length; i++) {
                (function getWidget(node) {
                    ...
                    if (node.attribs && node.attribs['lazo-widget'] && !node.attribs['lazo-widget'].children) {
                        expected++;
                        node.attribs.class = node.attribs.class ? node.attribs.class + ' unbound' : 'unbound';
                        node.attribs.class += ' rendering';
        ...
        _removeWidgets: function () {
            for (var k in this.widgetInstances) {
                for (var i = 0; i < this.widgetInstances[k].length; i++) {
                    this.widgetInstances[k].unbind(this.widgetInstances[k].el);
                }
            }
        }
