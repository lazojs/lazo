    ...
    return {
        ...
        serialize: function (ctl, rootCtx) {
            ...
            if (ctl.currentView && ctl.currentView) {
                ...
                if (ctl.currentView.itemView || ctl.currentView.itemViews) { // quack i am a collection view
                    _.each(['itemView', 'itemViews', 'emptyView', 'emptyView'], function (prop) {
                        if (ctl.currentView[prop]) {
                            serObj.currentView[prop] = ctl.currentView[prop];
                        }
                    });
                }
            }
