define(['underscore', 'utils/treeMixin'], function (_, tree) {
    ...
    return _.extend({
        ...
        _getNodesHtml: function (tree, callback) {
            ...
            function getNodeHtml(node, callback) {
                var html;

                if (self.getNodeType(node) === 'component') {
                    callback('<div ' + self._attrs.cmpName + '="' + node.name + '" ' + self._attrs.cmpId + '="' + node.cid + '"></div>');
                } else {
                    node.getHtml(function (err, html) {
                        if (err) {
                            throw err;
                        }
                        callback(html);
            ...
            (function processNode(node) {
                ...
                getNodeHtml(node, function (html) {
                    node.html = html;
                    getNodeHtml(node.currentView, function (html) {
